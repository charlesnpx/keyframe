...
2. **Run the command:**
   ...
   The automatic backend uses pinned MLX-Whisper on Apple Silicon running macOS 14 or newer and OpenAI Whisper elsewhere. Supported Macs resolve the exact pinned snapshot from the local cache before permitting a network download; unsupported platforms neither install MLX nor request its weights. Eligible automatic MLX failures retry OpenAI Whisper in a fresh CPU worker. Speaker detection is attempted by default when `HF_TOKEN` exists; pyannote then adds segment-level labels to Whisper segments. To enable it, accept the pyannote model terms at `https://huggingface.co/pyannote/speaker-diarization-community-1`, create a Hugging Face token at `https://huggingface.co/settings/tokens`, and export it as `HF_TOKEN`. If `HF_TOKEN` is missing or pyannote access fails, Keyframe warns and keeps the unlabeled Whisper transcript. The two exact known harmless pyannote warnings are condensed; unrelated warnings stay visible, and malformed diarization rows cannot be published.

   In automatic concurrency mode, accelerator transcription can overlap CPU diarization after 10%-headroom admission. macOS uses physical-memory pressure evidence with a bounded `vm_stat` fallback and never counts swap. A full run waits for transcription to release MPS/CUDA, then makes a fresh second-wave decision so frames may overlap remaining CPU diarization. Stages sharing one accelerator remain serialized.

   Reliable intervals select `max(T + F, D) + M + E`, `max(T, D) + F + M + E`, `T + max(D, F) + M + E`, `T + D + F + M + E`, or `T + F + M + E`, where `T`, `D`, `F`, `M`, and `E` are transcription, diarization, frames, transcript merge/output, and manifest enrichment/promotion.

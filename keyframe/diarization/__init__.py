"""Session-local diarization benchmark domain models."""

from keyframe.diarization.adapters import (
    BENCHMARK_RUN_RECORD_SCHEMA_VERSION,
    BenchmarkArtifactLayout,
    BenchmarkExecutionMode,
    BenchmarkRunRecord,
    DatasetAdapter,
    DatasetCacheConfig,
    DatasetExportResult,
    DatasetPreparationPlan,
    DatasetValidationResult,
    benchmark_run_record_from_dict,
    benchmark_run_record_json_dumps,
    benchmark_run_record_json_loads,
    build_artifact_layout,
    create_benchmark_run_record,
    ensure_adapter_cache_policy,
    plan_dataset_preparation,
    read_benchmark_run_record_json,
    write_benchmark_run_record_json,
)
from keyframe.diarization.ami import (
    AMI_BENCHMARK_BRANCHES,
    AMI_SAMPLE_RATE_HZ,
    AMIAdapter,
    AMIBenchmarkBranch,
    AMIBenchmarkBranchConfig,
    AMIChannelMetadata,
    AMIRecordingSource,
    AMISliceMetadata,
    AMISliceStatus,
    AMISpeakerSegment,
    AMISplitPlan,
    AMIWordAnnotation,
    ami_benchmark_branch_config,
    ami_dataset_snapshot_id,
    ami_run_record_identifiers,
    build_ami_candidate_bundle,
    build_ami_candidate_bundles,
    build_ami_reference_bundle,
    build_ami_slice_metadata,
    build_ami_split_plan,
    create_ami_benchmark_run_record,
    normalize_ami_recording,
    read_ami_channels_xml,
    read_ami_segments_xml,
    read_ami_words_xml,
)
from keyframe.diarization.attribution import apply_session_local_attribution
from keyframe.diarization.bundles import (
    CandidateBundle,
    CandidateBundleMode,
    ReferenceBundle,
    build_candidate_bundle,
    build_candidate_bundle_from_recording,
    validate_candidate_bundle_payload,
)
from keyframe.diarization.engines import (
    CannedJsonEngineAdapter,
    DiarizationEngineAdapter,
    EngineConfigMetadata,
    EngineContractStatus,
    EngineContractValidation,
    EngineRuntimeStatus,
    EngineRuntimeSupportStatus,
    EngineOutputKind,
    HostedProviderGovernance,
    HostedProviderJsonAdapter,
    HostedProviderKind,
    ModelArtifactGovernance,
    NormalizedEngineOutput,
    RawSpeakerEvidence,
    SelfHostedWhisperXPyannoteAdapter,
)
from keyframe.diarization.evaluator import (
    DiarizationEvaluationResult,
    DiarizationRecordingMetricRow,
    DiarizationSliceMetricRow,
    EvaluationInterval,
    EvaluationMetricStatus,
    EvaluationSliceDefinition,
    EvaluationSliceStatus,
    build_evaluation_slices,
    evaluate_diarization_candidate,
)
from keyframe.diarization.fixture_validation import (
    AudioTransformCacheCheck,
    FixtureIssueCategory,
    FixtureSliceMetadata,
    FixtureSliceStatus,
    FixtureValidationIssue,
    FixtureValidationResult,
    FixtureValidationStatus,
    build_fixture_slice_metadata,
    merge_fixture_validation_results,
    validate_audio_transform_cache,
    validate_candidate_bundle_against_reference,
    validate_canonical_recording,
    validate_canonical_reference_payload,
    validate_fixture_gate,
    validate_manifest_expected_files,
    validate_scoring_exports,
)
from keyframe.diarization.io import (
    canonical_json_dumps,
    canonical_json_loads,
    canonical_jsonl_dumps,
    canonical_jsonl_loads,
    read_recording_json,
    read_recordings_jsonl,
    recording_from_dict,
    recording_to_dict,
    validate_schema_version,
    write_recording_json,
    write_recordings_jsonl,
)
from keyframe.diarization.manifests import (
    ALLOWED_DATASET_ACCESS_MODES,
    ALLOWED_DATASET_ROLES,
    ALLOWED_SCORING_CHANNEL_MODES,
    ALLOWED_SCORING_POLICY_KINDS,
    ALLOWED_SCORING_SPEAKER_COUNT_MODES,
    ALLOWED_SCORING_TEXT_NORMALIZATION,
    ALLOWED_SCORING_UEM_REGIONS,
    DATASET_MANIFEST_SCHEMA_VERSION,
    DatasetAccess,
    DatasetAccessMode,
    DatasetManifest,
    DatasetRole,
    DatasetSplitManifest,
    ExpectedDatasetFile,
    ExpectedFileRole,
    RedistributionMode,
    ScoringChannelMode,
    ScoringPolicyManifest,
    ScoringPolicyKind,
    ScoringSpeakerCountMode,
    ScoringTextNormalization,
    ScoringUemRegions,
    dataset_manifest_from_dict,
    dataset_manifest_json_dumps,
    dataset_manifest_json_loads,
    dataset_manifest_to_dict,
    default_scoring_policy,
    default_scoring_policy_manifests,
    manifest_allows_default_ci_download,
    manifest_allows_default_full_download,
    read_dataset_manifest_json,
    read_scoring_policy_json,
    scoring_policy_from_dict,
    scoring_policy_hash,
    scoring_policy_json_dumps,
    scoring_policy_json_loads,
    scoring_policy_report_provenance,
    scoring_policy_to_dict,
    write_dataset_manifest_json,
)
from keyframe.diarization.models import (
    CanonicalRecording,
    CanonicalWord,
    ChannelRecord,
    DisplayLabel,
    ScoringRegion,
    SpeakerRecord,
    SpeakerSpan,
    TimeBasis,
    ValidationError,
)
from keyframe.diarization.monitoring import (
    MONITORING_RECORD_SCHEMA_VERSION,
    MonitoringAggregate,
    MonitoringPromotionState,
    MonitoringPromotionStep,
    MonitoringRecord,
    MonitoringRetentionClass,
    advance_monitoring_promotion_state,
    aggregate_monitoring_records,
    monitoring_promotion_state_from_dict,
    monitoring_record_from_dict,
    monitoring_record_json_dumps,
    monitoring_record_json_loads,
    read_monitoring_record_json,
    write_monitoring_record_json,
)
from keyframe.diarization.pipelines import (
    PIPELINE_BRANCH_IDS,
    BranchAcceptanceRecord,
    BranchPipelineResult,
    MonoMixBranchReport,
    PipelineBranchDecision,
    PipelineBranchId,
    build_asr_only_degraded_baseline,
    build_mono_mix_branch_report,
    build_pipeline_branch_evaluation_case,
    create_pipeline_branch_run_record,
    decide_mono_mix_branch_acceptance,
    render_branch_transcript,
    run_authenticated_track_metadata_branch,
    run_mono_mix_branch,
    run_separate_track_branch,
    validate_pipeline_branch_candidate_inputs,
    validate_pipeline_branch_payload,
)
from keyframe.diarization.private_acceptance import (
    ALLOWED_PRIVATE_ACCEPTANCE_LABELS,
    PRIVATE_ACCEPTANCE_METADATA_SCHEMA_VERSION,
    PRIVATE_ANNOTATION_QUALITY_GATE_STAGE,
    PrivateAcceptanceCoverageObservation,
    PrivateAcceptanceCoveragePlan,
    PrivateAcceptanceCoverageReport,
    PrivateAcceptanceCoverageSliceResult,
    PrivateAcceptanceCoverageSliceTarget,
    PrivateAcceptanceCoverageStatus,
    PrivateAcceptanceLabel,
    PrivateAcceptanceMetadata,
    PrivateAcceptanceSlice,
    PrivateAnnotationProtocol,
    PrivateAnnotationQualityGateConfig,
    PrivateAnnotationQualityGateResult,
    PrivateAnnotationQualityGateStatus,
    PrivateAnnotationQualityMetrics,
    evaluate_private_acceptance_coverage,
    evaluate_private_annotation_quality,
    private_acceptance_metadata_from_dict,
    private_acceptance_metadata_json_dumps,
    private_acceptance_metadata_json_loads,
    private_acceptance_metadata_to_dict,
    read_private_acceptance_metadata_json,
    write_private_acceptance_metadata_json,
)
from keyframe.diarization.preflight import (
    ALLOWED_PREFLIGHT_CAPTURE_MODES,
    ALLOWED_PREFLIGHT_ROUTES,
    PreflightCaptureMode,
    PreflightDiagnosticFeatureProvider,
    PreflightFeatures,
    PreflightJobRecord,
    PreflightManualOverrideAudit,
    PreflightPolicy,
    PreflightRoute,
    PreflightRouteAssessment,
    PreflightRouteConfusionReport,
    PreflightRouteDecision,
    build_preflight_route_confusion_report,
    preflight_decision_from_dict,
    preflight_features_from_dict,
    preflight_job_record_from_dict,
    preflight_manual_override_audit_from_dict,
    preflight_route_confusion_report_from_dict,
    render_transcript_for_preflight,
    route_preflight,
)
from keyframe.diarization.provenance import (
    ArtifactKind,
    AudioChannelMapping,
    AudioTimelineProvenance,
    AudioTransformConfig,
    AudioTransformManifest,
    DownmixPolicy,
    GainPolicy,
    NormalizedArtifactProvenance,
    OffsetMapSegment,
    TimelineMergeValidation,
    TimelineOffsetMap,
    TransformChain,
    TransformStep,
    TransformType,
    boundary_shift_degrades_scoring,
    build_audio_transform_manifest,
    build_mono_mix_transform_manifest,
    hash_audio_transform_config,
    normalize_transform_command,
    sha256_bytes,
    sha256_file,
    validate_timeline_merge,
)
from keyframe.diarization.rendering import (
    AssignSpanOverlay,
    MarkOverlapOverlay,
    MarkUncertainOverlay,
    MergeSpeakersOverlay,
    RenderedTranscript,
    RenderedTranscriptState,
    RenderedTurn,
    RenderedWord,
    RenameLabelOverlay,
    ReviewReason,
    SpeakerAttributionState,
    SplitSpeakerOverlay,
    TranscriptOverlay,
    rendered_transcript_json_dumps,
    render_transcript,
)
from keyframe.diarization.release import (
    RELEASE_RECORD_SCHEMA_VERSION,
    ReleaseApprovalStatus,
    ReleaseCandidateRecord,
    ReleaseGoldenTestResult,
    ReleaseGoldenTestStatus,
    ReleaseGovernanceDecision,
    ReleaseMustNotHaveChecks,
    ReleaseRevalidationReport,
    ReleaseRevalidationTrigger,
    ReleaseRouteState,
    ReleaseRuntimeAuditEvent,
    ReleaseRuntimeCheck,
    ReleaseRuntimeCheckStatus,
    ReleaseRuntimeConfig,
    check_release_runtime_config,
    read_release_record_json,
    release_expected_runtime_config,
    release_golden_test_result_from_dict,
    release_must_not_have_checks_from_dict,
    release_revalidation_report,
    release_revalidation_summary,
    release_record_content_hash,
    release_record_from_dict,
    release_record_json_dumps,
    release_record_json_loads,
    release_route_state_from_dict,
    release_runtime_config_from_dict,
    validate_release_runtime_output,
    write_release_record_json,
)
from keyframe.diarization.reports import (
    BENCHMARK_REPORT_SCHEMA_VERSION,
    BenchmarkEvaluationCase,
    BenchmarkGateConfig,
    BenchmarkMetricResult,
    BenchmarkRegressionBudget,
    BenchmarkReport,
    BenchmarkReportScopeType,
    BenchmarkReportStatus,
    CriticalSpanDiagnosticScore,
    CriticalSpanPolicyDefinition,
    MetricDirection,
    RegressionBudgetKind,
    RegressionGateResult,
    RegressionGateStatus,
    ReviewSignalCalibration,
    ReviewSignalSeverity,
    ReviewSignalSeverityBreakdown,
    ReviewSignalScopeCalibration,
    ReviewSignalSpan,
    UncertaintyInterval,
    UncertaintyStatus,
    benchmark_report_from_dict,
    benchmark_report_json_dumps,
    benchmark_report_json_loads,
    benchmark_report_to_markdown,
    build_benchmark_report,
    calibrate_review_signals,
    read_benchmark_report_json,
    score_diagnostic_critical_spans,
    write_benchmark_report_json,
    write_benchmark_report_markdown,
)
from keyframe.diarization.scoring_exports import (
    RttmPairScore,
    RttmRow,
    UemRow,
    recording_to_rttm_text,
    recording_to_uem_text,
    rttm_oracle_self_score,
    score_rttm_pair,
    scoring_regions_to_uem_text,
    speaker_spans_to_rttm_text,
    validate_rttm_text,
    validate_uem_text,
    write_rttm,
    write_uem,
)
from keyframe.diarization.sentinels import (
    SENTINEL_BASELINE_IDS,
    SentinelBaselineCheck,
    SentinelBaselineId,
    SentinelBaselineReport,
    SentinelBaselineStatus,
    build_sentinel_baseline_output,
    build_sentinel_baseline_outputs,
    evaluate_sentinel_baselines,
    require_passing_sentinel_baselines,
)
__all__ = [
    "CanonicalRecording",
    "CanonicalWord",
    "ChannelRecord",
    "DisplayLabel",
    "ScoringRegion",
    "SpeakerRecord",
    "SpeakerSpan",
    "TimeBasis",
    "ValidationError",
    "ArtifactKind",
    "AMI_BENCHMARK_BRANCHES",
    "AMIAdapter",
    "AMIBenchmarkBranch",
    "AMIBenchmarkBranchConfig",
    "AMIChannelMetadata",
    "AMIRecordingSource",
    "AMI_SAMPLE_RATE_HZ",
    "AMISliceMetadata",
    "AMISliceStatus",
    "AMISpeakerSegment",
    "AMISplitPlan",
    "AMIWordAnnotation",
    "BENCHMARK_REPORT_SCHEMA_VERSION",
    "BENCHMARK_RUN_RECORD_SCHEMA_VERSION",
    "AudioChannelMapping",
    "AudioTimelineProvenance",
    "AudioTransformCacheCheck",
    "AudioTransformConfig",
    "AudioTransformManifest",
    "AssignSpanOverlay",
    "BenchmarkArtifactLayout",
    "BenchmarkEvaluationCase",
    "BenchmarkExecutionMode",
    "BenchmarkGateConfig",
    "BenchmarkMetricResult",
    "BenchmarkRegressionBudget",
    "BenchmarkReport",
    "BenchmarkReportScopeType",
    "BenchmarkReportStatus",
    "BenchmarkRunRecord",
    "CandidateBundle",
    "CandidateBundleMode",
    "CannedJsonEngineAdapter",
    "PIPELINE_BRANCH_IDS",
    "PRIVATE_ACCEPTANCE_METADATA_SCHEMA_VERSION",
    "PRIVATE_ANNOTATION_QUALITY_GATE_STAGE",
    "RELEASE_RECORD_SCHEMA_VERSION",
    "ALLOWED_PRIVATE_ACCEPTANCE_LABELS",
    "ALLOWED_DATASET_ACCESS_MODES",
    "ALLOWED_PREFLIGHT_CAPTURE_MODES",
    "ALLOWED_PREFLIGHT_ROUTES",
    "ALLOWED_DATASET_ROLES",
    "ALLOWED_SCORING_CHANNEL_MODES",
    "ALLOWED_SCORING_POLICY_KINDS",
    "ALLOWED_SCORING_SPEAKER_COUNT_MODES",
    "ALLOWED_SCORING_TEXT_NORMALIZATION",
    "ALLOWED_SCORING_UEM_REGIONS",
    "DATASET_MANIFEST_SCHEMA_VERSION",
    "DatasetAccess",
    "DatasetAccessMode",
    "DatasetAdapter",
    "DatasetCacheConfig",
    "DatasetExportResult",
    "DatasetManifest",
    "DatasetPreparationPlan",
    "DatasetRole",
    "DatasetSplitManifest",
    "DatasetValidationResult",
    "DiarizationEngineAdapter",
    "DiarizationEvaluationResult",
    "DiarizationRecordingMetricRow",
    "DiarizationSliceMetricRow",
    "DownmixPolicy",
    "EngineConfigMetadata",
    "EngineContractStatus",
    "EngineContractValidation",
    "EngineRuntimeStatus",
    "EngineRuntimeSupportStatus",
    "EngineOutputKind",
    "HostedProviderGovernance",
    "HostedProviderJsonAdapter",
    "HostedProviderKind",
    "EvaluationInterval",
    "EvaluationMetricStatus",
    "EvaluationSliceDefinition",
    "EvaluationSliceStatus",
    "CriticalSpanDiagnosticScore",
    "CriticalSpanPolicyDefinition",
    "BranchAcceptanceRecord",
    "BranchPipelineResult",
    "MetricDirection",
    "MarkOverlapOverlay",
    "MarkUncertainOverlay",
    "MergeSpeakersOverlay",
    "MONITORING_RECORD_SCHEMA_VERSION",
    "ModelArtifactGovernance",
    "MonitoringAggregate",
    "MonitoringPromotionState",
    "MonitoringPromotionStep",
    "MonitoringRecord",
    "MonitoringRetentionClass",
    "NormalizedArtifactProvenance",
    "NormalizedEngineOutput",
    "OffsetMapSegment",
    "MonoMixBranchReport",
    "PipelineBranchDecision",
    "PipelineBranchId",
    "PrivateAcceptanceCoverageObservation",
    "PrivateAcceptanceCoveragePlan",
    "PrivateAcceptanceCoverageReport",
    "PrivateAcceptanceCoverageSliceResult",
    "PrivateAcceptanceCoverageSliceTarget",
    "PrivateAcceptanceCoverageStatus",
    "PrivateAcceptanceLabel",
    "PrivateAcceptanceMetadata",
    "PrivateAcceptanceSlice",
    "PrivateAnnotationProtocol",
    "PrivateAnnotationQualityGateConfig",
    "PrivateAnnotationQualityGateResult",
    "PrivateAnnotationQualityGateStatus",
    "PrivateAnnotationQualityMetrics",
    "PreflightCaptureMode",
    "PreflightDiagnosticFeatureProvider",
    "PreflightFeatures",
    "PreflightJobRecord",
    "PreflightManualOverrideAudit",
    "PreflightPolicy",
    "PreflightRoute",
    "PreflightRouteAssessment",
    "PreflightRouteConfusionReport",
    "PreflightRouteDecision",
    "ExpectedDatasetFile",
    "ExpectedFileRole",
    "FixtureIssueCategory",
    "FixtureSliceMetadata",
    "FixtureSliceStatus",
    "FixtureValidationIssue",
    "FixtureValidationResult",
    "FixtureValidationStatus",
    "GainPolicy",
    "RenderedTranscript",
    "RenderedTranscriptState",
    "RenderedTurn",
    "RenderedWord",
    "RawSpeakerEvidence",
    "ReferenceBundle",
    "RedistributionMode",
    "ReleaseApprovalStatus",
    "ReleaseCandidateRecord",
    "ReleaseGoldenTestResult",
    "ReleaseGoldenTestStatus",
    "ReleaseGovernanceDecision",
    "ReleaseMustNotHaveChecks",
    "ReleaseRevalidationReport",
    "ReleaseRevalidationTrigger",
    "ReleaseRouteState",
    "ReleaseRuntimeAuditEvent",
    "ReleaseRuntimeCheck",
    "ReleaseRuntimeCheckStatus",
    "ReleaseRuntimeConfig",
    "RegressionBudgetKind",
    "RegressionGateResult",
    "RegressionGateStatus",
    "RenameLabelOverlay",
    "ReviewSignalCalibration",
    "ReviewSignalSeverity",
    "ReviewSignalSeverityBreakdown",
    "ReviewSignalScopeCalibration",
    "ReviewSignalSpan",
    "RttmPairScore",
    "RttmRow",
    "ReviewReason",
    "ScoringChannelMode",
    "ScoringPolicyManifest",
    "ScoringPolicyKind",
    "ScoringSpeakerCountMode",
    "ScoringTextNormalization",
    "ScoringUemRegions",
    "SENTINEL_BASELINE_IDS",
    "SentinelBaselineCheck",
    "SentinelBaselineId",
    "SentinelBaselineReport",
    "SentinelBaselineStatus",
    "SelfHostedWhisperXPyannoteAdapter",
    "SpeakerAttributionState",
    "SplitSpeakerOverlay",
    "TimelineMergeValidation",
    "TimelineOffsetMap",
    "TranscriptOverlay",
    "TransformChain",
    "TransformStep",
    "TransformType",
    "UemRow",
    "UncertaintyInterval",
    "UncertaintyStatus",
    "advance_monitoring_promotion_state",
    "aggregate_monitoring_records",
    "apply_session_local_attribution",
    "boundary_shift_degrades_scoring",
    "benchmark_report_from_dict",
    "benchmark_report_json_dumps",
    "benchmark_report_json_loads",
    "benchmark_report_to_markdown",
    "benchmark_run_record_from_dict",
    "benchmark_run_record_json_dumps",
    "benchmark_run_record_json_loads",
    "ami_benchmark_branch_config",
    "ami_dataset_snapshot_id",
    "ami_run_record_identifiers",
    "build_ami_candidate_bundle",
    "build_ami_candidate_bundles",
    "build_ami_reference_bundle",
    "build_ami_slice_metadata",
    "build_ami_split_plan",
    "build_candidate_bundle",
    "build_candidate_bundle_from_recording",
    "build_artifact_layout",
    "build_audio_transform_manifest",
    "build_benchmark_report",
    "build_evaluation_slices",
    "build_fixture_slice_metadata",
    "build_asr_only_degraded_baseline",
    "build_mono_mix_branch_report",
    "build_mono_mix_transform_manifest",
    "build_pipeline_branch_evaluation_case",
    "build_preflight_route_confusion_report",
    "build_sentinel_baseline_output",
    "build_sentinel_baseline_outputs",
    "canonical_json_dumps",
    "canonical_json_loads",
    "canonical_jsonl_dumps",
    "canonical_jsonl_loads",
    "calibrate_review_signals",
    "check_release_runtime_config",
    "dataset_manifest_from_dict",
    "dataset_manifest_json_dumps",
    "dataset_manifest_json_loads",
    "dataset_manifest_to_dict",
    "default_scoring_policy",
    "default_scoring_policy_manifests",
    "evaluate_diarization_candidate",
    "evaluate_private_acceptance_coverage",
    "evaluate_private_annotation_quality",
    "evaluate_sentinel_baselines",
    "create_ami_benchmark_run_record",
    "create_benchmark_run_record",
    "create_pipeline_branch_run_record",
    "decide_mono_mix_branch_acceptance",
    "ensure_adapter_cache_policy",
    "manifest_allows_default_ci_download",
    "manifest_allows_default_full_download",
    "merge_fixture_validation_results",
    "monitoring_promotion_state_from_dict",
    "monitoring_record_from_dict",
    "monitoring_record_json_dumps",
    "monitoring_record_json_loads",
    "hash_audio_transform_config",
    "normalize_transform_command",
    "plan_dataset_preparation",
    "private_acceptance_metadata_from_dict",
    "private_acceptance_metadata_json_dumps",
    "private_acceptance_metadata_json_loads",
    "private_acceptance_metadata_to_dict",
    "preflight_decision_from_dict",
    "preflight_features_from_dict",
    "preflight_job_record_from_dict",
    "preflight_manual_override_audit_from_dict",
    "preflight_route_confusion_report_from_dict",
    "normalize_ami_recording",
    "read_ami_channels_xml",
    "read_ami_segments_xml",
    "read_ami_words_xml",
    "read_benchmark_report_json",
    "read_benchmark_run_record_json",
    "read_monitoring_record_json",
    "read_release_record_json",
    "read_recording_json",
    "read_dataset_manifest_json",
    "read_private_acceptance_metadata_json",
    "read_recordings_jsonl",
    "read_scoring_policy_json",
    "recording_from_dict",
    "recording_to_rttm_text",
    "recording_to_uem_text",
    "recording_to_dict",
    "release_expected_runtime_config",
    "release_golden_test_result_from_dict",
    "release_must_not_have_checks_from_dict",
    "release_revalidation_report",
    "release_revalidation_summary",
    "release_record_content_hash",
    "release_record_from_dict",
    "release_record_json_dumps",
    "release_record_json_loads",
    "release_route_state_from_dict",
    "release_runtime_config_from_dict",
    "render_transcript_for_preflight",
    "rendered_transcript_json_dumps",
    "render_branch_transcript",
    "render_transcript",
    "require_passing_sentinel_baselines",
    "route_preflight",
    "rttm_oracle_self_score",
    "run_authenticated_track_metadata_branch",
    "run_mono_mix_branch",
    "run_separate_track_branch",
    "score_rttm_pair",
    "score_diagnostic_critical_spans",
    "scoring_policy_from_dict",
    "scoring_policy_hash",
    "scoring_policy_json_dumps",
    "scoring_policy_json_loads",
    "scoring_policy_report_provenance",
    "scoring_policy_to_dict",
    "scoring_regions_to_uem_text",
    "sha256_bytes",
    "sha256_file",
    "speaker_spans_to_rttm_text",
    "validate_schema_version",
    "validate_audio_transform_cache",
    "validate_candidate_bundle_payload",
    "validate_candidate_bundle_against_reference",
    "validate_canonical_recording",
    "validate_canonical_reference_payload",
    "validate_fixture_gate",
    "validate_manifest_expected_files",
    "validate_pipeline_branch_candidate_inputs",
    "validate_pipeline_branch_payload",
    "validate_release_runtime_output",
    "validate_rttm_text",
    "validate_scoring_exports",
    "validate_uem_text",
    "validate_timeline_merge",
    "write_benchmark_run_record_json",
    "write_benchmark_report_json",
    "write_benchmark_report_markdown",
    "write_monitoring_record_json",
    "write_release_record_json",
    "write_recording_json",
    "write_recordings_jsonl",
    "write_dataset_manifest_json",
    "write_private_acceptance_metadata_json",
    "write_rttm",
    "write_uem",
]
